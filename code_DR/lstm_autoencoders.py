...
class LSTMAutoencoder_v4(nn.Module):
    ...
    def set_encode(self, new:bool):
        self.is_encode = new
    def set_decode(self, new:bool):
        self.is_decode = new

    def forward(self, inp, T_total=312):
        if self.is_encode:
            #print(inp.size())
            if inp.dim() != 3:
                #raise ValueError(f'Expected input to be 3-dimensional (batches, bins, time), got {inp.dim()}')
...
class LSTMAutoencoder_v4_demean(nn.Module):
    def __init__(self, input_size, cell_size, num_layers, segment_size=24):
        super(LSTMAutoencoder_v4_demean, self).__init__()
        self.segment_size = segment_size
        self.cell_size = cell_size
        self.is_decode = True
