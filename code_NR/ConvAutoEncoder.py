...
class Mel_AutoEncoder_final(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        # 1 x 1 x 840
        self.encode_bn = nn.Linear(in_features=840, out_features=420)
        # 1 x 1 x 420
        self.dropout = nn.Dropout(0.2)
        # 1 x 1 x 420
        self.decode_bn = nn.Linear(in_features=420, out_features=840)
        # 1 x 1 x 840
    ...
    def forward(self, x):
        ...
        #print(torch.min(x),torch.max(x),'decodeBN')

        x = torch.relu(self.decode_7(x))
        #print(torch.min(x),torch.max(x),'decode7')
        x = torch.reshape(x, (-1,8,12,35))
        #print(torch.min(x),torch.max(x),'reshape')
        x = self.decode_6(x, self.indices_3)
        #print(torch.min(x),torch.max(x),'decode6')
        x = torch.relu(self.decode_5(x))
        #print(torch.min(x),torch.max(x),'decode5')
        x = self.decode_4(x, self.indices_2)
        #print(torch.min(x),torch.max(x),'decode4') #no need for relu when unpooling
        x = torch.sigmoid(self.decode_3(x))
        #print(torch.min(x),torch.max(x),'decode3')
        x = self.decode_2(x, self.indices_1)
        #print(torch.min(x),torch.max(x),'decode2') #no need for relu when unpooling
        x = torch.sigmoid(self.decode_1(x))
        #print(torch.min(x),torch.max(x),'decode1')
...
class Mel_AutoEncoder_280_regularized(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.encode_bn = nn.Linear(in_features=420, out_features=280)
        # 1 x 1 x 280
        self.bottleneck_dropout = nn.Dropout(0.2)
    ...
    def forward(self, x):
        if self.encode:
            ...
            if self.training and self.decode:
        ...
        if self.decode:
            # DECODE
            x = torch.relu(self.decode_bn(x))
            if self.training and self.encode:
