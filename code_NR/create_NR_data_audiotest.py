...
def save_wav(signal, path):
    ...
    wavfile.write(path, 22050, signal)
    return
...
for active_speaker in range(4):
    print(f'+++CHAPTER {active_speaker}+++\n')
    for exp in range(1):
        print(f'\n\nExperiment {exp}:\n')
        ...
        savepath_prefix = '/home/student/Documents/WHK_Projekt_1/work_remastered/DATA/NR/NR_unfused_audiotest/'###############

        speaker = experiment_dict['speaker_data'][active_speaker]['speaker']
        print(f'Active speaker {active_speaker} | ID {speaker}')
        ...
        closest_clusters_dict = json.load(open('/home/student/Desktop/Code_BA_Intek/work/experiments/exp_'+str(exp)+'/closest_clusters.json'))
        closest_clusters = closest_clusters_dict['closest_clusters']

        clusters_dict = json.load(open('/home/student/Desktop/Code_BA_Intek/work/experiments//exp_'+str(exp)+'/clusters.json'))
        clusters = []
        for c in range(4):
        ...
        for seq in range(4):
            ...
            for input in range(4): #inputs for NR
                ...
                # node_signals_in_cc = torch.zeros((num_nodes_in_cc,16e4))
                    ...
                    for node in range(num_signals): #for every node of inputcluster
                        ...
                        node_signal_array[node] = node_signal_array[node] + node_signal[0:160000]

                for i in range(num_signals):
                    save_wav(node_signal_array[i], path_to_sample+f'/input{input}_node{i}.wav')
                print('+++++++++++++', type(target_audio))
                save_wav(target_audio, path_to_sample+'/target.wav')

                final_input = 0#feature_extraction(node_signal_tensor, 16e3, AE_model, exp, id_of_inputcluster, is_target=False)
                if not is_active:
            ...
            final_target = 0#feature_extraction(target_audio, 16e3, AE_model, is_target=True)
