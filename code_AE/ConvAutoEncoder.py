...
class Mel_AutoEncoder_280_regularized_2(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        # flatten
        # 1 x 1 x 3360
        self.encoder_dropout_1 = nn.Dropout(0.5)
        self.encode_7 = nn.Linear(in_features=3360, out_features=840)
        self.encoder_dropout_2 = nn.Dropout(0.5)
        # 1 x 1 x 840
        self.encode_8 = nn.Linear(in_features=840, out_features=420)
        self.encoder_dropout_3 = nn.Dropout(0.5)
        # 1 x 1 x 420
        self.encode_bn = nn.Linear(in_features=420, out_features=280)
        # 1 x 1 x 280
        self.bottleneck_dropout = nn.Dropout(0.2)
        ...
        self.decode_bn = nn.Linear(in_features=280, out_features=420)
        self.decoder_dropout_3 = nn.Dropout(0.5)
        # 1 x 1 x 420
        self.decode_8 = nn.Linear(in_features=420, out_features=840)
        self.decoder_dropout_2 = nn.Dropout(0.5)
        # 1 x 1 x 840
        self.decode_7 = nn.Linear(in_features=840, out_features=3360)#
        self.decoder_dropout_1 = nn.Dropout(0.5)
        # 1 x 1 x 3360
        # unflatten
        # 8 x 12 x 35
        self.decode_6 = nn.MaxUnpool2d(kernel_size=2, stride=2)
    ...
    def forward(self, x):
        if self.encode:
            ...
            if self.training and self.decode:
                x = self.encoder_dropout_2(x)

            x = torch.relu(self.encode_8(x))

            if self.training and self.decode:
                x = self.encoder_dropout_3(x)
        ...
        if self.decode:
            ...
            if self.training and self.encode:
                x = self.decoder_dropout_3(x)

            x = torch.relu(self.decode_8(x))

            if self.training and self.encode:
                x = self.decoder_dropout_2(x)

            x = torch.relu(self.decode_7(x))

            if self.training and self.encode:
                x = self.decoder_dropout_1(x)
